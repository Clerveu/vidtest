"""
VIDEO ANALYZER - Speaker Diarization Fork (OpenRouter Qwen3.5-397B-A17B)

Complete pipeline in one script:
1. Select video from Source/ with guessit-generated clean prefix
2. Extract audio, run WhisperX speaker diarization, split into 30s chunks
...
Requires: whisperx (with pyannote), HF_TOKEN env var
"""
...
# Venv Python for whisperx/pyannote (isolated to avoid package conflicts)
VENV_PYTHON = Path(__file__).parent / "subtest" / "venv" / "Scripts" / "python.exe"
DIARIZE_WORKER = Path(__file__).parent / "subtest" / "diarize_worker.py"
...
AUDIO_DIR = BASE_DIR / "audio"                    # extracted full audio tracks
...
# ============================================================================
# STEP 2: SPEAKER DIARIZATION (WhisperX + pyannote)
# ============================================================================

def find_english_audio_track(video_path):
    """Find the English audio track index in the video file."""
    cmd = [
    ...
    try:
        ...
        for i, s in enumerate(streams):
            ...
            if lang == "eng":
                print(f"  Audio track {i}: {s.get('codec_name')} | {lang}")
                return i

        # Single track — use it regardless of language tag
        if len(streams) == 1:
            s = streams[0]
            lang = s.get("tags", {}).get("language", "unknown")
            print(f"  Audio track 0 (only track): {s.get('codec_name')} | {lang}")
            return 0

        print("  No English audio track found!")
        return None
    except Exception as e:
        print(f"  Failed to probe audio tracks: {e}")
        return None

def extract_audio(video_path, prefix):
    """Extract full English audio track as 16kHz mono WAV for WhisperX."""
    AUDIO_DIR.mkdir(exist_ok=True)
    audio_path = AUDIO_DIR / f"{prefix}_audio.wav"

    if audio_path.exists():
        print(f"  Using existing audio: {audio_path.name}")
        return audio_path

    track_idx = find_english_audio_track(video_path)
    if track_idx is None:
        return None

    print(f"  Extracting full audio track...")
    cmd = [
...
def run_diarization(audio_path, prefix):
    """Run WhisperX diarization via venv subprocess, output chunks to SUBTITLES_DIR."""
    if not VENV_PYTHON.exists():
        print(f"  Venv Python not found: {VENV_PYTHON}")
        return False
    if not DIARIZE_WORKER.exists():
        print(f"  Diarize worker not found: {DIARIZE_WORKER}")
        return False
    ...
    print("  Launching diarization worker (venv subprocess)...")
    result = subprocess.run(cmd, env={**os.environ})

    if result.returncode != 0:
        print("  Diarization worker failed")
        return False
...
def main():
    ...
    print("  2. Speaker diarization (WhisperX + pyannote) -> 30s chunks")
    ...
    if not HAS_GUESSIT:
        print("NOTE: guessit not installed. Smart prefix generation disabled.")
        print("  Install with: pip install guessit")
        print()
    ...
    print_step(2, 7, "SPEAKER DIARIZATION")
    SUBTITLES_DIR.mkdir(exist_ok=True)
    ...
    if existing_sub_chunks:
        ...
        audio_path = extract_audio(video_path, prefix)
        if audio_path:
            if not run_diarization(audio_path, prefix):
                print("  Diarization failed. Continuing anyway...")
        else:
            print("  Audio extraction failed. Continuing without subtitles...")
